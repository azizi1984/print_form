...
jQuery(function ($) {
    var tableEl = $('#detail_declaration');
    ...
    if (createOrEditModal) {
        createOrEditModal.addEventListener('show.bs.modal', function (event) {
            ...
            if (button) {
                ...
                document.getElementById('create_or_edit_detail_template_id').value = id || '';
                document.getElementById('create_or_edit_detail_template_name').value = name || '';
                ...
                if (id) {
                    ...
                    if (labelEl) labelEl.innerHTML = '<i class="bi bi-pencil-square me-2 text-warning"></i>Edit Detail Template';
                    if (subtitleEl) subtitleEl.textContent = 'แก้ไขข้อมูลและสถานะเทมเพลตตารางรายการสินค้า';
                    document.getElementById('wrapper_created_at').style.display = 'block';
                } else {
                    // โหมด Create
                    document.getElementById('action_mode').value = 'create';
                    if (labelEl) labelEl.innerHTML = '<i class="bi bi-table me-2 text-primary"></i>Create Detail Template';
                    if (subtitleEl) subtitleEl.textContent = 'กำหนดชื่อและรายละเอียดเทมเพลตตารางรายการสินค้า (Detail Template) สำหรับใบขนสินค้าขาออก';
        ...
        if (btnSave) {
            btnSave.addEventListener('click', function () {
                ...
                const id = document.getElementById('create_or_edit_detail_template_id').value;
                const name = document.getElementById('create_or_edit_detail_template_name').value;
                ...
                if (!name.trim()) {
                    if (errorAlert && errorMessageEl) {
                        errorMessageEl.textContent = 'กรุณาระบุชื่อ Detail Template Name';
                        errorAlert.classList.remove('d-none');
                    } else {
                        alert('กรุณาระบุชื่อ Detail Template Name');
                    }
                    document.getElementById('create_or_edit_detail_template_name').focus();
                    return;
                }

                let url = '/ex-declaration/detail-template';
                let method = 'POST';

                if (mode === 'edit' && id) {
                    url = `/ex-declaration/detail-template/${id}`;
                    method = 'PUT';
                }
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        detail_template_name: name.trim(),
                        description: desc,
                        status: status
                    },
    ...
    $('#detail_declaration tbody').on('click', '.copy-btn', function (e) {
        e.preventDefault();
        ...
        if (confirm('Are you sure you want to duplicate this template?')) {
            ...
            $.ajax({
                url: `/ex-declaration/detail-template/${id}/copy`,
                type: 'POST',
                data: {
                    _token: $('meta[name="csrf-token"]').attr('content')
                },
    ...
    $('#detail_declaration tbody').on('click', '.delete-btn', function (e) {
        e.preventDefault();
        const id = $(this).attr('data-id');

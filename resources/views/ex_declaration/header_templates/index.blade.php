...
@section('page-title', 'Export Declaration Header Templates - Print Form')
    ...
    <div class="card-header bg-white py-3 d-flex flex-wrap justify-content-between align-items-center gap-2">
        <div>
            <h5 class="mb-0 fw-bold text-dark d-flex align-items-center">
                <i class="bi bi-layout-text-window me-2 text-primary"></i> Export Declaration Header Templates
            </h5>
            <small class="text-muted">จัดการและกำหนดรูปแบบเทมเพลตส่วนหัวเอกสารใบขนสินค้าขาออก</small>
        </div>
        <button type="button" class="btn btn-primary btn-sm px-3 shadow-sm rounded-pill" data-bs-toggle="modal" data-bs-target="#createOrEditModal">
            <i class="bi bi-plus-lg me-1"></i> Create Header Template
        </button>
    </div>
    <div class="card-body">
        <div class="table-responsive">
            <table id="header_declaration" class="table table-hover align-middle" style="width:100%">
                <thead>
                    <tr>
                ...
                <tbody>
                    @foreach($headerTemplates as $row)
                    <tr>
                        <td class="text-center text-muted fw-semibold">{{ $loop->iteration }}</td>
                        <td>
                            <span class="fw-semibold text-dark">{{ $row->header_template_name ?? $row['header_template_name'] }}</span>
                        </td>
                        ...
                        <td class="text-center text-muted" style="font-size: 13px;">
                            {{ ($row->created_at ?? $row['created_at']) ? \Carbon\Carbon::parse($row->created_at ?? $row['created_at'])->format('d/m/Y H:i') : '-' }}
                        </td>
                        ...
                        <td>
                            <div class="d-flex justify-content-center align-items-center flex-nowrap gap-1">
                                <a href="{{ route('header-template.show', $row->header_template_id ?? $row['header_template_id']) }}" class="btn-action btn-action-view" title="จัดการ Layout">
                                    <i class="bi bi-layout-text-window"></i>
                                </a>
                                <a href="#" class="btn-action btn-action-copy copy-btn" title="คัดลอกเทมเพลต" data-id="{{ $row->header_template_id ?? $row['header_template_id'] }}">
                                    <i class="bi bi-copy"></i>
                                </a>
                                <a href="#" class="btn-action btn-action-edit" title="แก้ไขข้อมูล"
                                   data-bs-toggle="modal" 
                                   data-bs-target="#createOrEditModal"
                                   data-id="{{ $row->header_template_id ?? $row['header_template_id'] }}"
                                   data-name="{{ $row->header_template_name ?? $row['header_template_name'] }}"
                                   data-desc="{{ $row->description ?? $row['description'] ?? '' }}"
                                   data-status="{{ $row->status ?? $row['status'] }}"
                                   data-date="{{ ($row->created_at ?? $row['created_at']) ? \Carbon\Carbon::parse($row->created_at ?? $row['created_at'])->format('d/m/Y H:i') : '' }}">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <a href="#" class="btn-action btn-action-delete delete-btn" title="ลบ" data-id="{{ $row->header_template_id ?? $row['header_template_id'] }}">
        ...
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
                ...
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        ...
                        <small class="text-muted" id="editModalSubtitle">
                            กำหนดชื่อและรายละเอียดเทมเพลตส่วนหัวเอกสาร (Header Template) สำหรับใบขนสินค้าขาออก
                        </small>
                    </div>
                </div>
            ...
            <div class="modal-body p-4">
                ...
                <form id="createOrEditForm">
                    <input type="hidden" id="create_or_edit_header_template_id" name="header_template_id">
                    <input type="hidden" id="action_mode" name="action_mode" value="create">

                    <!-- Template Name Field -->
                    <div class="mb-3">
                        <label for="create_or_edit_header_template_name" class="form-label fw-semibold text-dark d-flex justify-content-between align-items-center">
                            <span><i class="bi bi-tag text-primary me-1"></i> Header Template Name <span class="text-danger">*</span></span>
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control border-start-0 ps-1" id="create_or_edit_header_template_name" name="header_template_name" placeholder="เช่น Standard Export Header Template" required autocomplete="off">
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-text text-muted" style="font-size: 12px;">
                            <i class="bi bi-pencil me-1"></i>ระบุข้อมูลสั้น ๆ เพื่อช่วยจดจำวัตถุประสงค์หรือประเภทการใช้งานของเทมเพลต
                        </div>
                    </div>
...
@push('scripts')
    @vite(['resources/js/ex_declaration/header_script.js'])
@endpush

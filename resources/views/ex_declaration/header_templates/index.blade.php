            ...
            <table id="header_declaration" class="table table-hover align-middle" style="width:100%">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="fw-semibold text-dark">{{ $row['header_template_name'] }}</td>
                        ...
                        <td>
                            <div class="d-flex justify-content-center align-items-center flex-nowrap gap-1">
                                <a href="{{ route('header-template.show', $row['header_template_id']) }}" class="btn-action btn-action-view" title="จัดการ Layout">
                                    <i class="bi bi-layout-text-window"></i>
                                </a>
                                <a href="#" class="btn-action btn-action-copy copy-btn" title="คัดลอกเทมเพลต" data-id="{{ $row['header_template_id'] ?? '' }}">
                                    <i class="bi bi-copy"></i>
                                </a>
                                <a href="#" class="btn-action btn-action-edit" title="แก้ไขข้อมูล"
                                   data-bs-toggle="modal" 
                                   data-bs-target="#createOrEditModal"
                                   data-id="{{ $row['header_template_id'] ?? '' }}"
                                   data-name="{{ $row['header_template_name'] ?? '' }}"
                                   data-desc="{{ $row['description'] ?? '' }}"
                                   data-status="{{ $row['status'] ?? '' }}"
                                   data-date="{{ $row['created_at'] ?? '' }}">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <a href="#" class="btn-action btn-action-delete delete-btn" title="ลบ" data-id="{{ $row['header_template_id'] ?? '' }}">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content border-0">
            <div class="modal-header">
                <h5 class="modal-title fw-bold" id="editModalLabel"><i class="bi bi-journal-text me-2 text-primary"></i>Create / Edit Header Template</h5>
            ...
            <div class="modal-body">
                <form id="createOrEditForm">
                    ...
                    <div class="mb-3">
                        <label for="create_or_edit_header_template_name" class="form-label fw-semibold">Header Template Name <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="create_or_edit_header_template_name" name="header_template_name" required>
                    </div>
                    <div class="mb-3">
